...
&emsp;&emsp;&emsp;&emsp; 🎯 Взглянем на примере
...
&emsp;&emsp;&emsp;&emsp; 🎯 И при этом нужно сохранить стандартную навигацию в браузере    
...
👆🏽 Модалки управляемые роутером бывают двух видов   

🔹 Зависимые   
&emsp;&emsp; 🎯 Зависят от своих родительских компонентов, и могут быть доступными только из них  
&emsp;&emsp; 🎯 Рассмотрим на примере, т.е модалка с данными юзера может быть открыта по url только из страници пользователей  

🔹 Общедоступные   
&emsp;&emsp; 🎯 Находяться на общем темплейте сайта, и могут быть вызваны из любого места  
&emsp;&emsp; 🎯 Рассмотрим на примере, модалка с авторизацией, она доступна отовсюду и имеет свою ссылку на открытое состояние

<br>

🔹 Я предлагаю погрузиться в IDE и рассмотреть как все это работает на примере реального кода

💠 Начнем с зависимых модалок  

🔹 Зависимые модалки реализуються через знакомый нам компонент `<Route>`  
...
&emsp;&emsp; 🎯 Для зависимых модалок хук `usePrepareLink`, принимает на вход два параметра(`{to, isRelativePath}`)

&emsp;&emsp; 🎯 Свойство `to` описывает по какой ссылке мы хотим видить открытую модалку

&emsp;&emsp; 🎯 Свойство `isRelativePath` говорит о том что мы хотим прикрепить `to`, к нашему текущему `url`

&emsp;&emsp; 🎯 Глянем как работает хук

&emsp;&emsp; 🎯 Если в крации, то хук подстегивает к урлу текущей странички пропс `to:` и возвращает готовый роут, который мы видем в url при открытии модалки
...
&emsp;&emsp; 🎯 1. При открытии, `url` меняеться сразу, а модалка отображаеться с анимацией

&emsp;&emsp; 🎯 2. При закрытии, `url` меняеться сразу, модалка скрываеться с анимацией

&emsp;&emsp; 🎯 3. Процессы открытия и закрытия модалки не должны сохраняться в браузерной истроии
...
&emsp;&emsp; 🎯 А так же содержит в своих пропсах свойство `match`, которое будет пустым, если текущий `url` не совпадает с указанным в пропсе `path`
...
&emsp;&emsp; 🔹 Для начала нужно создать конфиг таких модалок, он состоит из следующих составляющих (открыть app components/get/const)

&emsp;&emsp;&emsp;&emsp; 🎯 Ожидаемые гет параметры которые будут передаваться

&emsp;&emsp;&emsp;&emsp; 🎯 Енам значений этих гет параметров, который помогает более удобно к ним обращаться и вести учет значений

&emsp;&emsp;&emsp;&emsp; 🎯 Список модалок которые будут открыты при передаче определенных get параметров
...
&emsp;&emsp;&emsp;&emsp; 🎯 В отличии от манипуляции с комопнентом `<Route>`, тут происходит мгновенное размонтирование компонента, и нам нужно позаботиться об анимации скрытия

<br>

&emsp;&emsp; 🔹 Для этого мы используем хук `useGetPopupState`

&emsp;&emsp;&emsp;&emsp; 🎯 Он будет решать когда монтировать и размонтировать модалку, и передавать ей пропс `isOpened`, для отрабатывания анимации

(спускаемся в хук)

&emsp;&emsp;&emsp;&emsp; 🎯 Получаем значение указанного гет параметра(в нашем случаи это параметра popup)

&emsp;&emsp;&emsp;&emsp; 🎯 Если в гет параметрах был указанный ключ, мы просто записываем ее значение, и компонент выше вмонтируем модалку из списка по ключу в дом 

(показать пример)

&emsp;&emsp;&emsp;&emsp; 🎯 А если гет параметр вызывающий модалку был удален из `url`, срабатывает хук мемо, который `isOpened = false`, и запускаем очистку имени модалки с таймаутом в `500ms`
...
&emsp;&emsp; 🔹 Для отображения общедоступными модалок мы используем все тот же хук `usePrepareLink` только уже с другимми парраметрами
...
&emsp;&emsp;&emsp;&emsp; 🎯 Тут мы передаем в query ключь который мы храним в константах отвечающий за то что должна отображаться модалка  
